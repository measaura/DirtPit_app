...
export default class MotoTourScreen extends Component {
	constructor(props) {
		super(props);
...
//         this._hideDateTimePicker();
    ...
    filterField() {
        ...
        let passedBike = this.state.bikefield;

			console.log('zonedone',this.state.selectedDateStar)
			console.log('package',this.state.timeslotfield)

				if (passedDate == "Select Date") {
            Alert.alert("Please select Date.");
        } else if (!passedSlot) {
            Alert.alert("Please select Tour Package.");
        } else {
        	this.validate()
        }
    }
...
//         return fetch("https://ftwventures.com.my/index.php?route=api/country/countries", {
			...
			.then(json => {
			console.log('json',json)
				const biketemp = json.bikes.map(
					(item) => ({
							label: item.name,
							value: item.bike_id
					})
				)
				console.log('bikes',biketemp)
				this.setState({
					bikeoption: biketemp 
				})
			})
					...
					/>
                    	...
                    	<TouchableOpacity 
							...
							<View style={{height:400}}>
											<Text style={{alignSelf: 'center', fontFamily:'Gotham-Bold', fontSize: 20, marginTop: 10}}>MX Tours</Text>
											<TouchableOpacity
										...
										<DropDownPicker
												ref="bikefieldRef"
												items={this.state.bikeoption}
												placeholder="Select MX Bike"
												containerStyle={{height:50, width: 300, marginTop: 10, alignSelf: 'center'}}
												style={{backgroundColor:'#cdcdcd',zIndex:1000,borderTopLeftRadius: 25, borderTopRightRadius: 25, borderBottomLeftRadius: 25, borderBottomRightRadius: 25}}
												dropDownStyle={{backgroundColor: '#cdcdcd'}}
												labelStyle={{
